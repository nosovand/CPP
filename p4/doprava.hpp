...
struct StDepart : public Depart
{
    int ln; // line number
    int conn; // connection order

    // Constructors
    StDepart(const Depart &dep, int ln_ind, int conn_ind);
    StDepart(const StDepart &sd);
    // Assign
    StDepart &operator=(const StDepart &d);
};
...
struct DepartDB
{
    multimap<string, StDepart> db;
    vector<StDepart> getStDeparts(const string &stop) const; // get vector of StDepartures for given stop
    void loadFromFile(string filename);
};
