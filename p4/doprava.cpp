...
StDepart::StDepart(const Depart &dep, int ln_ind, int conn_ind) : Depart::Depart(dep.ti, dep.dir), ln(ln_ind), conn(conn_ind) {}

StDepart::StDepart(const StDepart &stdep) : StDepart::StDepart((Depart)stdep, stdep.ln, stdep.conn) {}

StDepart &StDepart::operator=(const StDepart &d)
{
    this->ti = d.ti;
    this->dir = d.dir;
    this->ln = d.ln;
...
void Network::loadFromFile(string filename)
{
    ...
    if (infile.is_open())
    {
        ...
        infile.close();
        cout << "Loaded network from " << filename << ".\n";
    }
    else
    {
        cout << "Could not open " << filename << ". Network not loaded." << endl;
    }
}

vector<StDepart> DepartDB::getStDeparts(const string &stop) const
{
    vector<StDepart> v;
    for_each(db.lower_bound(stop), db.upper_bound(stop), [&](const pair<string, StDepart> &x) { v.push_back(x.second); });
    return v;
}

void DepartDB::loadFromFile(string filename)
{
    string tmpstr;
    ifstream infile(filename);
    if (infile.is_open())
    {
        while (getline(infile, tmpstr))
        {
            ...
            this->db.insert(make_pair(key, StDepart(Depart(Time(sec), dir), ln, ord)));
        }
        infile.close();
        cout << "Loaded departure database from " << filename << ".\n";
    }
    else
    {
        cout << "Could not open " << filename << ". Departure database not loaded." << endl;
    }
}